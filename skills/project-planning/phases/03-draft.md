...
Per-milestone issue bodies. The `linear-issue` skill is the **source of truth** for everything about an issue body: shape, sections, grounding, estimation, quality gates. Invoke it once at phase entry and follow whatever it says — Phase 3 never re-describes its rules. Phase 3 owns the surrounding orchestration: single invocation, plan-metadata wrapping, per-milestone batching, review loop.

## Entry conditions

- Phase 2 shape locked (Step C sign-off approved).
- Per-milestone issue trees available in chat context.
- **If Phase 1 Step 4 ran**, code-explorer findings remain in chat context. Use them for issue-body technical grounding (existing patterns to extend, prior implementations to cite, subtle dependencies) and as input to `linear-issue`'s estimation rationale.

---

## Steps

### 1. Invoke `linear-issue` once

At phase entry, invoke the skill via the `Skill` tool with `skill: linear-issue`. One invocation only — its loaded content stays in chat context for the rest of Phase 3. Do **not** re-invoke per issue.

If `linear-issue` requires per-issue choices that should be consistent across this plan, resolve them once at phase entry and apply uniformly to every issue — do not let `linear-issue` ask per invocation.
...
2. **Draft every issue in the milestone in one response.** For each issue, run the `linear-issue` skill end to end on that issue, then wrap its output in the Phase 3 plan-metadata block (template below). Apply whatever quality gates `linear-issue` defines before presenting.
...
**Estimate rationale**: base <N> + <adjuster, e.g. "+2 new schema"> - <reducer, e.g. "-1 reused calc helper">; <one-line summary>. (Defer to `linear-issue`'s estimation rules for the actual values.)

#### Linear issue (uploaded in Phase 4)

<`linear-issue` output verbatim — whatever Title + body the skill produced>
```
